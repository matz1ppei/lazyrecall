...
- **Daily Session** — Review → Reverse Review → Match Madness → Blank Fill の4フェーズを1セットで実施
- **4択 Review** — 単語を見て意味を4択で選ぶ
- **Reverse Review** — 意味を見て単語を4択で選ぶ
...
```bash
git clone https://github.com/ippei/lazyrecall
cd lazyrecall
...
1日1セットが目標。**Review → Reverse Review → Match Madness → Blank Fill** の4フェーズを同じ20枚のカードで実施。

- 4フェーズすべて正解（Quad-correct）→ `UpdateReview(Good)` でインターバルが進む
